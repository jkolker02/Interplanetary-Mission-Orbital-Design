...
%PLOTS TRANFER ORBIT EARTH TO VENUS
[t,y]= ode45(ode2,timer,X, options); %Runs the ODE calculation numerically using a reference to the drdr function, time span, and initial conditions
...
timed = [0,15113520]; %Time from venus to mars in seconds
X4 = [posV,v1D3]; %Initilal position and velocity of the satelite leaving venus
[t4,y4] = ode45(ode2, timer, X4, options);
...
view(0,89.9)
for k = 1:500;
    ixr = round(k/500 * length(xr));
    ixr2 = round(k/500 * length(xr2));
    ixr3 = round(k/500 * length(xr3));
    ixr4 = round(k/500 * length(xr4));
    ixr5 = round(k/500 * length(xr5));
    addpoints(h,xr(ixr),yr(ixr), zr(ixr));
    addpoints(h2,xr2(ixr2),yr2(ixr2), zr2(ixr2));
    addpoints(h3,xr3(ixr3),yr3(ixr3), zr3(ixr3));
    addpoints(h4,xr4(ixr4),yr4(ixr4), zr4(ixr4));
